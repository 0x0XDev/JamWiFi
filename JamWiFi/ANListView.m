...
- (void)scanInBackground {
	...
	dispatch_async(queue, ^{
			...
			self->interfaceName = CWInterface.interface.interfaceName;
			...
			if (!foundNets) {
				NSLog(@"wifi scan error");
				
				// Cleanup
				_close(airportHandle);
				...
				[weakSelf performSelectorOnMainThread:@selector(handleScanSuccess:) withObject:networks waitUntilDone:NO];
			}
